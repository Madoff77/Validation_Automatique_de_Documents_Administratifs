...
import {
  ArrowLeft, Building2, Edit3, Save, X, Wand2, Upload,
  FileText, AlertTriangle, CheckCircle, Loader2, RefreshCw,
} from 'lucide-react'
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <p className="text-sm text-gray-900">{value || <span className="text-gray-400 italic">Non renseigné</span>}</p>
      )}
    </div>
  )
}
...
function ExtractedChip({ label, value }) {
  ...
  return (
    <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 rounded-lg px-3 py-1.5">
      ...
      <span className="text-xs text-blue-800 font-semibold truncate max-w-40">{value}</span>
    </div>
  )
}
...
export default function SupplierDetail() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (e) => toast.error(e.response?.data?.detail || 'Erreur de mise à jour'),
  })
  ...
  const handleAutoFill = () => {
    const processed = docs.filter((d) => d.status === 'processed' && d.doc_type)
    if (!processed.length) {
      toast.error('Aucun document traité disponible pour l\'auto-remplissage')
      return
    }

    // Récupérer les données complètes du premier doc traité de chaque type
    const promises = processed.slice(0, 5).map((d) => documentsApi.get(d.document_id))
    setAutoFilling(true)
    ...
    Promise.allSettled(promises).then((results) => {
      ...
      results.forEach((r) => {
        if (r.status === 'fulfilled' && r.value.extracted) {
          ...
          if (ext.tva_number && !merged.tva_number) merged.tva_number = ext.tva_number
          if (ext.raison_sociale && !merged.name) merged.name = ext.raison_sociale
          if (ext.adresse && !merged.address) merged.address = ext.adresse
        }
      })
      ...
      setEditing(true)
      setAutoFilling(false)
      toast.success(`${Object.keys(merged).length} champ(s) auto-rempli(s) depuis les documents`)
    })
  }
  ...
  const startEdit = () => { setEditForm({ ...supplier }); setEditing(true) }
  const cancelEdit = () => { setEditForm({}); setEditing(false) }
  ...
  const pendingDocs = docs.filter((d) => !['processed', 'error'].includes(d.status))

  return (
    <div className="p-8 max-w-5xl mx-auto">
      {/* Breadcrumb */}
      <Link to="/suppliers" className="inline-flex items-center gap-1.5 text-sm text-gray-500
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <ComplianceBadge status={supplier.compliance_status} />
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="col-span-2">
                <Field label="Raison sociale" value={supplier.name}
                       editValue={editForm.name} onChange={setField('name')} editing={editing} />
              </div>
              <Field label="SIRET" value={supplier.siret} editValue={editForm.siret}
                     onChange={setField('siret')} editing={editing} placeholder="14 chiffres" />
              <Field label="N° TVA" value={supplier.tva_number} editValue={editForm.tva_number}
                     onChange={setField('tva_number')} editing={editing} placeholder="FR12345678901" />
              <Field label="Email" value={supplier.email} editValue={editForm.email}
                     onChange={setField('email')} editing={editing} />
              <Field label="Téléphone" value={supplier.phone} editValue={editForm.phone}
                     onChange={setField('phone')} editing={editing} />
              <div className="col-span-2">
                <Field label="Adresse" value={supplier.address} editValue={editForm.address}
                       onChange={setField('address')} editing={editing} />
              </div>
              <div className="col-span-2">
                <Field label="Notes" value={supplier.notes} editValue={editForm.notes}
                       onChange={setField('notes')} editing={editing} />
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-5">
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Statut conformité</h3>
            <ComplianceBadge status={supplier.compliance_status} size="md" />
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-500">Docs traités</span>
                <span className="font-medium text-gray-900">{processedDocs.length}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-500">En traitement</span>
                <span className="font-medium text-gray-900">{pendingDocs.length}</span>
              </div>
            </div>
          </div>

          {/* Données extraites synthèse */}
          {processedDocs.length > 0 && (
            <div className="card p-5">
              ...
              <p className="text-xs text-gray-400 mb-3">
                Depuis {processedDocs.length} document{processedDocs.length > 1 ? 's' : ''} traité{processedDocs.length > 1 ? 's' : ''}
              </p>
              <div className="space-y-2">
                {supplier.siret && <ExtractedChip label="SIRET" value={supplier.siret} />}
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2 text-xs text-gray-500">
              <div>
                <span className="block text-gray-400">Créé le</span>
                <span>{format(new Date(supplier.created_at), 'dd/MM/yyyy HH:mm')}</span>
              </div>
              <div>
                <span className="block text-gray-400">Modifié le</span>
                <span>{format(new Date(supplier.updated_at), 'dd/MM/yyyy HH:mm')}</span>
              </div>
            </div>
          </div>
        </div>
